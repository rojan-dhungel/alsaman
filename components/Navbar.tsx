  ...
  return (
    <nav className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[95%] max-w-7xl">
      <div className="bg-white/80 backdrop-blur-xl border border-white/20 shadow-soft rounded-[2rem] px-6 py-3 transition-all duration-300">
            ...
            <div className="relative group px-4 py-2 cursor-pointer flex items-center gap-1 group/nav">
              ...
              <div className="absolute top-full left-0 pt-4 opacity-0 translate-y-2 invisible group-hover:opacity-100 group-hover:translate-y-0 group-hover:visible transition-all duration-300">
                <div className="bg-white rounded-2xl shadow-xl border border-border overflow-hidden min-w-[280px]">
                  {categories.length > 0 ? (
                    categories.map((cat) => (
                      <DropdownLink key={cat.id} href={`/products/${cat.slug}`}>
                        {cat.name.toUpperCase()}
                      </DropdownLink>
                    ))
                  ) : (
                    <div className="px-6 py-4 text-sm text-secondary animate-pulse">Loading categories…</div>
                  )}
                </div>
              </div>
            </div>
        ...
        {/* Mobile Menu */}
        <div className={`lg:hidden transition-all duration-300 overflow-hidden ${isMobileMenuOpen ? "max-h-[600px] mt-4 opacity-100" : "max-h-0 opacity-0"}`}>
          <div className="flex flex-col space-y-2 pb-4">
            ...
            {categories.map((cat) => (
              <MobileNavLink
                ...
                className="pl-8 text-sm opacity-80"
...
function NavLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link href={href} className="relative px-4 py-2 group transition-all duration-300 text-primary-dark/70 hover:text-accent">
...
function DropdownLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link href={href} className="block px-6 py-3 text-sm text-text-secondary hover:bg-bg-main hover:text-primary transition-colors border-b last:border-0 border-border/50">
      {children}
    </Link>
  );
}
...
function MobileNavLink({ href, children, className = "", onClick }: { href: string; children: React.ReactNode; className?: string; onClick?: () => void }) {
  return (
    <Link href={href} onClick={onClick} className={`block px-4 py-3 rounded-2xl hover:bg-primary/5 text-text-primary font-medium transition-colors ${className}`}>
      {children}
    </Link>
  );
}
